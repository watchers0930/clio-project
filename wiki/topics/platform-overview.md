...
사이드바(`src/components/layout/sidebar.tsx`)에 고정된 9개 메뉴:
...
| 4 | 템플릿 | `/templates` | 문서 생성용 템플릿 관리 |
| 5 | 문서 생성 | `/documents` | 초안 편집 모달 / 완료 문서는 전용 뷰어로 이동 |
| 6 | 계약 리스크 | `/contract-risk` | 계약서 업로드 → AI 리스크 조항 분석 |
| 7 | 메시지 | `/messages` | DM/채널, 첨부파일, Supabase Realtime; 미읽음 배지 (10초 폴링) |
| 8 | 일정/할일 | `/schedule` | 월간 캘린더 + CRUD |
| 9 | 설정 | `/settings` | 부서/사용자 관리, RBAC 역할 설정 |

> **결재 메뉴 없음** — v6.2.0에서 완전 제거. 사이드바 navItems에 결재 관련 항목 없음.
