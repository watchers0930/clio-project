...
#### 상단 정보 (반드시 채우세요)
- "작성자 소속" 옆 빈칸 → "부서명" 값
- "작성자 직급" 옆 빈칸 → 지시사항의 직급 값 (없으면 빈 문자열)
- "작성자명" 옆 빈칸 → "작성자" 값
- "회의일시" 또는 "회의 일시" 옆 빈칸 → "작성일자" 값 (예: 2026-04-05)
- "회의시간" 또는 "회의 시간" 옆 빈칸 → 지시사항의 시간 값
- "장소" 옆 빈칸 → 지시사항의 장소 값
- "부서" 옆 빈칸 → "부서명" 값
- "작성자" 옆 빈칸 → "작성자" 값
- "안건" 옆 빈칸 → 지시사항의 안건 내용
...
- "회의내용" 또는 "회의 내용" 영역 빈칸 → 지시사항의 모든 회의 내용을 상세히 기술. 마침표(.) 또는 쉼표(,) 뒤에 반드시 \\n을 넣어 줄바꿈 하세요.
- "비고" 열 빈칸 → 지시사항에 "비고:" 로 명시된 내용만 넣고, 없으면 반드시 빈 문자열("")
#### 결정사항/결과
- "결정사항" 또는 "회의 결과" 영역 빈칸 → 결정된 사항들. 항목별로 한 행씩 배치.
- "진행일정" 열 빈칸 → 각 결정사항의 일정/기한
- "특이사항" 영역 빈칸 → 특별히 언급할 사항, 없으면 빈 문자열("")
#### 참석자 테이블 (소속/성명/연락처/서명 열이 있는 경우)
중요! 참석자 데이터가 "이름(직급/부서/연락처)" 형식이면:
  - "소속" 열 → **부서명** (개발팀, 기획팀 등). 절대 사람 이름을 넣지 마세요!
  - "성명" 열 → **사람 이름** (김동의, 이수진 등). 절대 직급을 넣지 마세요!
  - "연락처" 열 → 반드시 빈 문자열(""). 절대 값을 넣지 마세요.
  - "서명" 열 → 반드시 빈 문자열("")
예시: "김동의(대표/개발팀/010-1234-5678)" → 소속="개발팀", 성명="김동의", 연락처="", 서명=""

### 보고서
#### 상단 메타데이터 (프로그래밍으로 채워지므로 빈 문자열 유지)
- "보고처" 또는 "보고처(부서)" 옆 빈칸 → 빈 문자열("") (프로그래밍 후처리)
- "보고서명" 옆 빈칸 → 빈 문자열("") (프로그래밍 후처리)
- "회의 일자" 또는 "회의일자" 옆 빈칸 → 빈 문자열("") (프로그래밍 후처리)
- "장소" 옆 빈칸 → 지시사항의 장소 값
- "참석자" 옆 빈칸 → 지시사항의 참석자 이름만 (쉼표 구분, 직급/부서 제외)
- "취급" 옆 빈칸 → 빈 문자열("")
- "기밀·보통" 옆 빈칸 → 빈 문자열("")
#### 정보(자료) 출처
- 참조 자료(소스 파일)의 파일명 목록을 줄바꿈(\\n)으로 나열
- 참조 자료가 없으면 지시사항에 언급된 자료명을 나열
#### 보고 내용과 의견 (핵심 영역 — AI가 상세하게 작성)
- 참조 자료의 내용을 분석하고, 지시사항의 요청 사항에 맞게 AI 의견을 작성
- 예: "마케팅 방안을 마련해줘" → 참조 자료를 근거로 마케팅 전략/방안을 상세히 기술
- 마침표(.) 뒤에 \\n을 넣어 줄바꿈
- 최소 200자 이상 상세하게 작성
#### 문제점
- 참조 자료와 지시사항을 바탕으로 AI가 판단한 예상 문제점/리스크를 기술
- 항목별로 \\n 줄바꿈
- 문제점이 없어도 최소 1개 이상 잠재적 리스크를 언급
...
- "보고 내용과 의견", "회의내용", "내용" 등 큰 영역은 반드시 200자 이상 상세하게 작성하세요.
