...
## 분량 기준
- **전체 제안서: 6000~8000자**
- 각 H3 항목은 서술형(2~4문단)으로 작성합니다. 불릿만 나열하지 마세요.
- 표가 적절한 곳에는 4~6행의 표를 포함합니다.
...
- **3.2 세부 수행 내용**: 핵심 모듈을 ####(H4)로 분리, 모듈마다 기능 목록 표 + 처리 흐름([A]→[B]→[C] 형태)을 포함합니다.
- **3.3 시스템 구성도**: 텍스트 기반 아키텍처 다이어그램.
- **3.4 데이터 흐름**: 텍스트 플로우차트 + 외부 연동 표.
