...
export async function generateDocumentContent(params: {
  ...
  const templateFileLen = templateFileText?.length ?? 0;
  const maxSourceChars = MAX_CONTEXT_CHARS - templateFileLen;
...
${templateFileText ? '\n중요: "표준양식 파일 구조"가 제공된 경우, 해당 양식의 섹션 구조, 항목 순서, 표 형식을 최대한 따라서 작성하세요. 양식에 있는 빈칸/플레이스홀더는 참조 자료를 바탕으로 실질적인 내용으로 채워주세요.' : ''}`;
  ...
  userPrompt += `위 참조 자료를 바탕으로 "${templateName}" 문서를 완성해주세요.${templateFileText ? ' 표준양식의 구조와 형식을 최대한 유지하세요.' : ''}`;
