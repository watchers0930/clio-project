...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // 본인 확인
    const { data: existing } = await admin.from('events').select('created_by').eq('id', id).single();
    if (!existing || existing.created_by !== authUserId) {
      return NextResponse.json({ success: false, error: '권한 없음' }, { status: 403 });
    }

    const updates: Record<string, unknown> = {};
    for (const key of ['title', 'description', 'location', 'event_type', 'start_at', 'end_at', 'all_day', 'department_id']) {
    ...
    if (error) {
      console.error('[events/PATCH]', error.message);
