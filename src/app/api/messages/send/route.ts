...
export async function POST(request: NextRequest) {
  try {
    ...
    const { channelId, content, documentId } = body;

    if (!channelId) {
      return NextResponse.json({ success: false, error: '채널 ID가 필요합니다.' }, { status: 400 });
    }
    ...
    if (error) {
      console.error('[messages/send/POST]', error.message);
