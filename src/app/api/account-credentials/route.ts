...
type CredRow = { id: string; site_name: string; username: string; enc_password: string; created_at: string };
...
export async function GET() {
  ...
  const { data, error } = await (supabase as any)
    ...
    .select('id, site_name, username, enc_password, created_at')
    .eq('user_id', userId)
...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await (admin as any)
    .from('account_credentials')
    .insert({ user_id: userId, site_name: site_name.trim(), username: username.trim(), enc_password })
    .select('id, site_name, username, created_at')
    .single();

  if (error) return NextResponse.json({ error: (error as { message: string }).message }, { status: 500 });

  const row = data as { id: string; site_name: string; username: string; created_at: string };
