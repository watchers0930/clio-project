...
function FilesPage() {
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <input
            ...
            placeholder="파일 검색..."
            className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-[#e5e5e7] bg-white text-sm text-[#1d1d1f] placeholder:text-[#6e6e73] focus:outline-none focus:ring-2 focus:ring-[#0071e3]"
          />
        </div>
        ...
        <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }} className="px-3 py-2.5 rounded-xl border border-[#e5e5e7] bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[#0071e3]">
          {STATUSES.map((s) => <option key={s}>{s}</option>)}
        </select>

        {/* view toggle */}
        <div className="flex rounded-xl border border-[#e5e5e7] overflow-hidden ml-auto">
          <button onClick={() => setView('list')} className={`px-3 py-2 text-sm ${view === 'list' ? 'bg-[#1d1d1f] text-white' : 'bg-white text-[#6e6e73] hover:bg-[#f5f5f7]'} transition-colors`}>
          ...
          <button onClick={() => setView('grid')} className={`px-3 py-2 text-sm ${view === 'grid' ? 'bg-[#1d1d1f] text-white' : 'bg-white text-[#6e6e73] hover:bg-[#f5f5f7]'} transition-colors`}>
      ...
      {/* result count */}
      <p className="text-sm text-[#6e6e73]">총 <span className="font-num font-medium text-[#1d1d1f]">{filtered.length}</span>개 파일</p>
