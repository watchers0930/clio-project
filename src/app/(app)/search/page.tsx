  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }} className="pb-10">
      ...
      <button
        ...
        className="fixed bottom-8 right-8 z-40 w-14 h-14 rounded-full bg-[#0071e3] text-white shadow-lg hover:bg-[#005bbf] transition-all flex items-center justify-center"
        title="AI에게 질문하기"
      >
        {chatOpen ? (
      ...
      {/* ── 채팅 패널 ── */}
      {chatOpen && (
        <div className="fixed bottom-28 right-8 z-40 w-96 bg-white rounded-2xl border border-[#e5e5e7] shadow-2xl flex flex-col overflow-hidden" style={{ height: 520 }}>
          {/* 헤더 */}
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#e5e5e7] bg-[#f5f5f7] shrink-0">
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {chatMessages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full text-center gap-3">
                <svg className="w-10 h-10 text-[#0071e3] opacity-60" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" />
                </svg>
                <p className="text-sm text-[#6e6e73]">문서에 대해 질문해 보세요</p>
                <div className="flex flex-col gap-2 w-full mt-1">
                  {['납기일이 언제인가요?', '계약 금액이 얼마인가요?', '담당자 이름이 뭔가요?'].map((q) => (
                    <button
                      key={q}
                      onClick={() => { setChatInput(q); }}
                      className="text-left text-xs text-[#0071e3] bg-[#f0f7ff] hover:bg-[#e0f0ff] px-3 py-2 rounded-xl transition-colors"
                    >
                      {q}
                    </button>
                  ))}
          ...
          {/* 입력창 */}
          <div className="px-4 py-3 border-t border-[#e5e5e7] shrink-0 flex gap-2">
            <input
              type="text"
              value={chatInput}
              ...
              className="flex-1 rounded-xl border border-[#e5e5e7] bg-[#f5f5f7] text-sm text-[#1d1d1f] placeholder:text-[#a1a1a6] focus:outline-none focus:ring-2 focus:ring-[#0071e3] disabled:opacity-50 px-3 py-2.5"
            />
            <button
              onClick={sendChat}
              disabled={chatLoading || !chatInput.trim()}
              className="w-10 h-10 rounded-xl bg-[#0071e3] text-white flex items-center justify-center hover:bg-[#005bbf] transition-colors disabled:opacity-40 shrink-0"
            >
