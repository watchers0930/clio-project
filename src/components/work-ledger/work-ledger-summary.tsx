...
export function WorkLedgerSummary({ projects }: Props) {
  ...
  const cards = [
    { label: '진행중', value: `${s.activeCount}건`, icon: Briefcase, color: '#2E6FF2', href: undefined as string | undefined },
    { label: '진행예정', value: `${s.plannedCount}건`, icon: CalendarClock, color: '#0EA5E9', href: undefined },
    { label: '총 계약액', value: formatKRW(s.totalContract), icon: Wallet, color: '#12805C', href: undefined },
    { label: '예상수익 합계', value: formatKRW(s.totalExpectedProfit), icon: TrendingUp, color: '#7C3AED', href: '/work-ledger/stats' },
    {
        ...
        if (c.href) {
          return (
            <Link
              key={c.label}
              href={c.href}
              className="group rounded-xl border border-border bg-white px-5 py-6 transition-colors hover:border-primary hover:bg-surface-secondary/40"
            >
              {inner}
              <p className="mt-1 text-[11px] text-primary opacity-0 transition-opacity group-hover:opacity-100">매출 통계 보기 →</p>
            </Link>
          );
        }
