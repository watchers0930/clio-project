...
const PRESET_ICONS: Record<string, ReactNode> = {
  file: (
    <svg className="w-10 h-10 text-[#a1a1a6]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.2}>
...
export function EmptyState({ icon, iconType, title, description, action, className }: EmptyStateProps) {
  ...
  return (
    <div className={cn('flex flex-col items-center justify-center py-20 text-center', className)}>
      ...
      {description && (
        <p className="text-sm text-[#6e6e73] max-w-xs leading-relaxed">{description}</p>
      )}
      {action && (
        <button
          onClick={action.onClick}
          className={cn(
            'mt-5 px-5 py-2.5 rounded-xl text-sm font-medium transition-colors',
