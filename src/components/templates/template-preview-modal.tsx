    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="template-preview-title">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-6xl mx-4 max-h-[88vh] overflow-y-auto">
        <div className="px-8 py-6 border-b border-border flex items-center justify-between">
          <div>
            <h2 id="template-preview-title" className="text-lg font-semibold text-foreground">{template.name}</h2>
            <p className="mt-1 text-sm text-foreground-secondary">{template.description || '템플릿 미리보기'}</p>
          </div>
        ...
        <div className="px-8 py-5 border-t border-border flex justify-end gap-3">
          <button onClick={onClose} className="px-6 py-3 rounded-xl border border-border text-sm text-foreground-secondary hover:bg-surface-secondary transition-colors">
            닫기
          </button>
          <button onClick={() => onEdit(template)} className="px-6 py-3 rounded-xl bg-foreground text-white text-sm font-medium hover:bg-primary transition-colors">
            템플릿 수정
          </button>
        </div>
      </div>
