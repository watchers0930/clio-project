    ...
    <div className="flex flex-col h-full bg-white border-l border-border">
      ...
      <div className="px-5 py-4 border-b border-border flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-foreground">AI 품질 검수</span>
        </div>
      ...
      {/* 점수 요약 */}
      {result && (
        <div className="px-5 py-3 bg-surface-tertiary border-b border-border shrink-0">
          <p className={`text-xs font-medium ${ScoreColor(result.overall_score)}`}>
            {ScoreLabel(result.overall_score)}
          </p>
          {result.summary && (
            <p className="text-xs text-foreground-secondary mt-1 leading-relaxed">{result.summary}</p>
          )}
        </div>
      )}

      {/* 카테고리 탭 */}
      <div className="px-4 py-3 border-b border-border flex gap-2 flex-wrap shrink-0">
        {TABS.map((tab) => {
          ...
          return (
            ...
            >
              {tab.label}
              {result && count > 0 && (
                <span className={`ml-1 ${activeTab === tab.id ? 'text-white/70' : 'text-primary'}`}>
                  [{count}]
                </span>
              )}
            </button>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4">
        ...
        {status === 'loading' && (
          <div className="flex flex-col items-center justify-center py-12 gap-3">
            ...
            <p className="text-xs text-foreground-secondary">GPT-4o가 문서를 검수하고 있습니다...</p>
          </div>
        )}
        ...
        {status === 'idle' && !result && (
          <div className="flex flex-col items-center justify-center py-12 gap-3 text-center">
            <svg className="w-10 h-10 text-border-secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
      ...
      {/* 하단 액션 */}
      <div className="px-5 py-4 border-t border-border shrink-0 space-y-3.5">
        ...
        <div className="flex gap-2">
          {result && (
            ...
            className="flex-1 px-4 py-2.5 rounded-xl border border-border text-xs text-foreground-secondary hover:bg-surface-secondary disabled:opacity-40 transition-colors"
            >
              재검수
            </button>
          )}
          <button
            ...
            disabled={status === 'loading'}
            className="flex-1 px-4 py-2.5 rounded-xl bg-primary text-white text-xs font-medium hover:bg-primary disabled:opacity-40 transition-colors"
          >
